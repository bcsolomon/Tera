...
A C table UDF is driven by the runtime through a series of phases. The function calls
`FNC_GetPhase(&phase)` (or `FNC_GetPhaseEx`) to learn the current **mode** and **phase**:

- **`TBL_PRE_INIT`**: establish global context; in constant mode optionally call
  `FNC_TblControl()` to become the controlling copy. Cannot build rows.
- **`TBL_INIT`**: open external connections; in constant mode a copy may `FNC_TblOptOut()`.
  Cannot build rows.
- **`TBL_BUILD`**: fill the output arguments to build one row, then return; the runtime calls
  `TBL_BUILD` again for the next row. Set `sqlstate` to `"02000"` to signal no more rows.
- **`TBL_FINI`** (variable mode only): close per-input-row connections; the runtime may loop
  back to `TBL_INIT` for the next input row.
- **`TBL_END`** / **`TBL_ABORT`**: close all connections and release allocated memory.
