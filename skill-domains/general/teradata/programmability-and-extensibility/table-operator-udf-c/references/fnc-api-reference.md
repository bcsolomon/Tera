...
The contract function defines the output schema. The operator function iterates the input and
output streams. Stream handles expose the current row via `handle->row->columnptr[i]`,
`handle->row->lengths[i]`, and the null indicators (`TBLOPISNULL`, `TBLOPSETNULL`).

| Function | Phase | Description |
|---|---|---|
| `FNC_TblOpGetStreamCount(&in, &out)` | both | Number of input and output streams. |
| `FNC_TblOpGetColCount(streamNo, ISINPUT)` | both | Column count of a stream. |
| `FNC_TblOpGetColDef(streamNo, ISINPUT, defs)` | both | Get input column definitions with UTF-8 names. |
| `FNC_TblOpGetBaseInfo(cols, baseInfos)` | both | Get UDT base type metadata for the columns. |
| `FNC_TblOpSetOutputColDef(streamNo, defs)` | contract | Define output columns for the dynamic schema. |
| `FNC_TblOpSetFormat(key, streamNo, dir, val, size)` | contract | Set the row and field format (for example INDICFMT1). |
| `FNC_TblOpSetContractDef(buf, len)` | contract | Stash a structure passed to the operator at runtime. |
| `FNC_TblOpOpen(streamNo, 'r' or 'w', 0)` | operator | Open a stream for read or write. Returns a handle. |
| `FNC_TblOpRead(handle)` | operator | Read the next row. Returns `TBLOP_SUCCESS` or `TBLOP_EOF`. |
| `FNC_TblOpWrite(outHandle)` | operator | Write the current output row. |
| `FNC_TblOpClose(handle)` | operator | Close a stream. |
| `FNC_TblOpIsDimension(streamNo, ISINPUT)` | operator | Whether a stream is a DIMENSION input. |
| `FNC_TblOpGetHashByDef()` | contract | Get HASH BY clause column definitions. |
| `FNC_TblOpSetHashByDef(numCols, defs)` | contract | Set HASH BY clause column definitions. |
| `FNC_TblOpGetLocalOrderByDef()` | contract | Get LOCAL ORDER BY clause columns. |
| `FNC_TblOpSetLocalOrderByDef(numCols, defs)` | contract | Set LOCAL ORDER BY clause columns. |
| `FNC_TblOpGetCustomKeyInfoAt(index, keyBuf, size, &numValues)` | contract | Get custom clause key name by index. |
| `FNC_TblOpGetCustomKeyInfoOf(key, &numValues)` | contract | Get value count for a named custom key. |
| `FNC_TblOpGetCustomValuesOf(key, valueBuf, size, valueIndex)` | contract | Get custom clause value at index. |
...
| Aggregate phases (`FNC_GetPhase`) | Yes | No | Aggregate UDFs only |
| `FNC_CallSP` | Yes | Yes | |
| CLI (`DBCHCL`/`DBCAREA`) | No | Yes | CXSPs only |
| C++ wrappers (`Connection`/`Statement`) | No | Yes | CXSPs only |
