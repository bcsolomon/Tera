...
Same as Java SP mappings — see `teradata-stored-procedures/references/java-stored-procedures.md` for the complete SQL-to-Java type table. For **scalar** Java UDFs the method **returns** the mapped type and all parameters are IN-only. For **table** Java UDFs the method returns `void` and has one output **array** parameter per `RETURNS TABLE` column (see the Table Functions section below).
...
Table UDFs are phase-driven, not "mode 0/1/2". Each AMP runs the method repeatedly across a
set of phases; the framework reports a **mode** (constant vs variable) plus the current phase.

**C** uses `FNC_GetPhase(&phase)` and `FNC_Phase` constants `TBL_PRE_INIT`, `TBL_INIT`,
`TBL_BUILD`, `TBL_FINI` (variable mode), `TBL_END`, `TBL_ABORT` (and `TBL_BUILD_EOF` via
`FNC_GetPhaseEx`). Build a row in `TBL_BUILD`; signal no-more-rows by `strcpy(sqlstate, "02000")`.

**Java** constructs `com.teradata.fnc.Tbl` and calls `getPhase(int[] phase)` (returns the mode,
writes the phase into `phase[0]`). The method returns `void` with one output array parameter per
`RETURNS TABLE` column. Build a row by assigning each output `[0]` during `Tbl.TBL_BUILD`;
signal no-more-rows by throwing `SQLException` with SQLState `"02000"`. Retain state between
iterations with `Tbl.allocCtx()` plus `getCtxObject()` / `setCtxObject()`. Constant mode adds
`Tbl.control()` (controlling copy) and `Tbl.optOut()`; variable mode adds the `TBL_FINI` phase.
