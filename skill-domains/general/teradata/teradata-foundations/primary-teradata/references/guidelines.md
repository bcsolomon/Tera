...
**Start here for UAF:** [uaf-concepts.md](uaf-concepts.md) — covers SERIES_SPEC, MATRIX_SPEC, ART_SPEC, the `EXECUTE FUNCTION INTO ART` execution pattern, ART layers, and TD_EXTRACT_RESULTS.
...
For full EXPLAIN interpretation guidance, optimization playbook, and stats collection patterns: [query-tuning.md](query-tuning.md).
...
- See [open-table-format-setup.md](open-table-format-setup.md) before writing any OTF DDL or DML
...
- See [object-store-read.md](object-store-read.md) before writing any NOS SQL
...
If you are unsure whether a native function exists for an operation, consult the topic index and check.
